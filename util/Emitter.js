...
export default class Emitter {
  ...
  emit(event, payload) {
    ...
    if (Array.isArray(handlers)) {
      // 如果某种事件有监听的处理函数，那么一定在事件名组成的数组里
      for (let it of handlers) {
        // 遍历当前事件的处理函数
        if (!it.invalid && typeof isFunction(it.handler)) {
          // 如果被标记为不合法的，或者说当前项内没有有效的函数，则跳过
          it.handler(payload) // 一一调用对应的处理函数，并把负荷捎带给它们
          if (it.once) {
            // 如果一个处理函数被声明为只调用一次，那么先标记为不合法，稍后处理。
      ...
      this[HANDLERS][event] = this[HANDLERS][event].reduce((pub, it) => {
        // pub即为reduce函数的第二个参数，在这里最开始时是[]
        if (!it.invalid) {
          // 如果是合法的，则收集它们
          pub.push(it)
        }
